...
public class AmenitiesDAOImpl implements AmenitiesDAO {
	...
	public boolean addAmenity(Amenities amenity) {
		final String SQL="insert into amenities(amenityName, amenityCredit) values (?,?)";
		try{
			System.out.println("Trying Connection");
			conn=connectionDB.createConnection();
			try {
		...
		try{
			...
			try {
				...
				while (rs.next()) {
					...
					ameenityItem.setCost(rs.getInt("amenityCredit"));
		...
		try{
			...
			try {
				...
				if (rs.next()) {
					...
					amenity.setCost(rs.getInt("amenityCredit"));}
				}
			catch(SQLException e) {
				e.printStackTrace();
			}
	...
	public Amenities getAmenityInfoByName(String amenityName) {
		Amenities amenity =new Amenities();
		final String SQL="select * from amenities where amenityName=?";
		try{
			System.out.println("Trying Connection");
			conn=connectionDB.createConnection();
			try {
